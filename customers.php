...
<style>
    ...
    .customer-dashboard .page-head h1{margin:0;font-size:1.8rem;font-weight:700}
    ...
    .customer-dashboard .filter-grid{display:grid;grid-template-columns:2.1fr 1fr 1fr 1fr 1fr;gap:16px}
    ...
    .customer-dashboard .form-control,.customer-dashboard .form-select{border-radius:10px;border:1px solid #dbe1ea;padding:.72rem .9rem;box-shadow:none}
    ...
    .customer-dashboard .filter-actions{display:flex;justify-content:flex-end;gap:12px;flex-wrap:wrap;margin-top:18px}
    .customer-dashboard .table-card{overflow:hidden}
    .customer-dashboard .table-shell{overflow:auto}
    .customer-dashboard .customer-table{margin:0;min-width:1180px}
    ...
    .customer-dashboard .status-pill{display:inline-flex;align-items:center;border-radius:999px;padding:.34rem .72rem;font-size:.74rem;font-weight:700}
    ...
    .customer-dashboard .icon-btn{display:inline-flex;align-items:center;justify-content:center;width:36px;height:36px;border-radius:10px;border:1px solid #dbe1ea;background:#fff;color:#334155;text-decoration:none}
    .customer-dashboard .icon-btn:hover{background:var(--accent-soft);border-color:#a5b4fc;color:var(--accent-dark)}
    .customer-dashboard .pagination-bar{display:flex;justify-content:space-between;align-items:center;gap:16px;flex-wrap:wrap;padding:18px 20px}
    .customer-dashboard .per-page-form{display:flex;align-items:center;gap:10px;color:var(--muted)}
    .customer-dashboard .per-page-form .form-select{width:auto;min-width:92px;padding:.55rem 2rem .55rem .8rem}
    .customer-dashboard .pagination{margin:0}
    .customer-dashboard .page-link{border:none;border-radius:10px;color:#334155;padding:.55rem .85rem}
    .customer-dashboard .page-item.active .page-link{background:var(--accent);color:#fff}
    .customer-dashboard .page-link:hover{background:#eef2ff;color:#4338ca}
    ...
    @media (max-width:1199px){.customer-dashboard .filter-grid{grid-template-columns:repeat(3,minmax(0,1fr))}}
    @media (max-width:991px){.customer-dashboard .filter-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
    @media (max-width:575px){.customer-dashboard .filter-grid{grid-template-columns:1fr}.customer-dashboard .page-head{align-items:stretch}.customer-dashboard .page-head a{width:100%}.customer-dashboard .filter-actions .btn{width:100%}.customer-dashboard .pagination-bar{align-items:flex-start}}
</style>
        ...
        <div class="saas-card table-card">
            <?php if (empty($customers)): ?>
                <div class="empty-state">
                    ...
                    <p>Try clearing the search, widening the date range, or removing one of the active filters.</p>
                        ...
                        <tbody>
                            <?php foreach ($customers as $customer): ?>
                                ...
                                $loyaltyText = $loyaltyTier !== '' && strcasecmp($loyaltyTier, 'none') !== 0
                                    ? $loyaltyTier
                                    : ($loyaltyPoints > 0 ? $loyaltyPoints . ' pts' : 'None');
                                $loyaltySub = $loyaltyPoints > 0 ? number_format($loyaltyPoints) . ' pts' : null;
                                ?>
                                <tr>
                                    <td>
                                        <div class="primary-text"><?php echo escape($fullName); ?></div>
                                        <div class="secondary-text"><?php echo escape((string)$customer['customer_code']); ?></div>
                                    </td>
                                    <td>
                                        <div><?php echo escape((string)($customer['email'] ?: 'No email')); ?></div>
                                        <?php if (!empty($customer['phone'])): ?>
                                            <div class="secondary-text"><?php echo escape((string)$customer['phone']); ?></div>
                                        <?php endif; ?>
                                    </td>
                                    <td><?php echo escape($customerType); ?></td>
                                    <td class="money"><?php echo formatCurrency($customer['total_spent']); ?></td>
                                    <td class="numeric"><?php echo number_format((int)$customer['num_transactions']); ?></td>
                                    <td>
                                        <?php echo $customer['last_purchase'] ? escape(date('Y-m-d', strtotime((string)$customer['last_purchase']))) : '-'; ?>
                                    </td>
                                    <td>
                                        <div><?php echo escape($loyaltyText); ?></div>
                                        <?php if ($loyaltySub && strcasecmp($loyaltyText, $loyaltySub) !== 0): ?>
                                            <div class="secondary-text"><?php echo escape($loyaltySub); ?></div>
                                        <?php endif; ?>
                                    </td>
                                    <td><span class="status-pill status-<?php echo $statusKey; ?>"><?php echo ucfirst($statusKey); ?></span></td>
                                    <td class="action-cell">
                                        <a href="<?php echo getBaseUrl(); ?>/customer_profile.php?id=<?php echo (int)$customer['id']; ?>" class="icon-btn" title="View">
                                            <i class="bi bi-eye"></i>
                                        </a>
                                        <a href="<?php echo getBaseUrl(); ?>/customer_form.php?id=<?php echo (int)$customer['id']; ?>" class="icon-btn ms-1" title="Edit">
                ...
                <div class="pagination-bar border-top">
                    ...
                    <nav aria-label="Pagination">
                        <ul class="pagination">
                            ...
                            <li class="page-item">
                                <a class="page-link" href="<?php echo getBaseUrl(); ?>/customers.php?<?php echo buildQueryString($filters, ['page' => 1]); ?>">First</a>
                            </li>
                            <li class="page-item">
                                <a class="page-link" href="<?php echo getBaseUrl(); ?>/customers.php?<?php echo buildQueryString($filters, ['page' => $filters['page'] - 1]); ?>">Previous</a>
                            </li>
                            <?php endif; ?>
                            <?php for ($i = max(1, $filters['page'] - 2); $i <= min($total_pages, $filters['page'] + 2); $i++): ?>
                            <li class="page-item <?php echo $i === $filters['page'] ? 'active' : ''; ?>">
                                <a class="page-link" href="<?php echo getBaseUrl(); ?>/customers.php?<?php echo buildQueryString($filters, ['page' => $i]); ?>"><?php echo $i; ?></a>
                            </li>
                            <?php endfor; ?>
                            <?php if ($filters['page'] < $total_pages): ?>
                            <li class="page-item">
                                <a class="page-link" href="<?php echo getBaseUrl(); ?>/customers.php?<?php echo buildQueryString($filters, ['page' => $filters['page'] + 1]); ?>">Next</a>
                            </li>
                            <li class="page-item">
                                <a class="page-link" href="<?php echo getBaseUrl(); ?>/customers.php?<?php echo buildQueryString($filters, ['page' => $total_pages]); ?>">Last</a>
                            </li>
                            <?php endif; ?>
                        </ul>
                    </nav>
...
function buildQueryString($filters, $overrides = []) {
    ...
    $params = array_filter($params, function($v) {
        return $v !== '' && $v !== null && $v !== 'asc' && $v !== 'name' && $v !== 10;
    });
